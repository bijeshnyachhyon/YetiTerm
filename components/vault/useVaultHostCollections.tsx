...
import { STORAGE_KEY_DISMISSED_KNOWN_HOSTS } from "../../infrastructure/config/storageKeys";
...
export function useVaultHostCollections({
  ...
  const handleDeleteKnownHost = useCallback((id: string) => {
    const target = knownHostsRef.current.find((kh) => kh.id === id);
    if (!target) {
      onUpdateKnownHostsRef.current(
    ...
    const targetHostname = target.hostname.trim().toLowerCase();

    // Record dismissed hostname so auto-scan doesn't re-import it
    try {
      const dismissed = new Set(
        localStorageAdapter.read<string[]>(STORAGE_KEY_DISMISSED_KNOWN_HOSTS, []) || [],
      );
      dismissed.add(targetHostname);
      if (target.port) {
        dismissed.add(`${targetHostname}:${target.port}`);
      }
    ...
    onUpdateKnownHostsRef.current(
      knownHostsRef.current.filter((kh) => {
        if (kh.id === id) return false;
        if (targetHostname === "(hashed)") {
          return kh.hostname.trim().toLowerCase() !== "(hashed)";
        }
        if (kh.hostname.trim().toLowerCase() === targetHostname) {
          if (!target.port || !kh.port || kh.port === target.port) {
            return false;
          }
        }
        return true;
