...
test("top tabs keeps quick controls independent and removes cloud sync button", () => {
  assert.doesNotMatch(topTabsSource, /<SyncStatusButton/);
  assert.match(topTabsSource, /TopTabsQuickControls/);
  assert.match(topTabsSource, /onClick=\{onOpenSettings\}/);
...
test("TopTabs wires onCreateLocalTerminal and opens AI panel on click", () => {
  assert.match(topTabsSource, /onCreateLocalTerminal\?: \(\) => string \| void/);
  assert.match(topTabsSource, /onCreateLocalTerminal\?\.SessionId|onCreateLocalTerminal\?\.()/);
  assert.match(topTabsSource, /handleToggleAiAssistant/);
  assert.match(topTabsSource, /prev\.onCreateLocalTerminal === next\.onCreateLocalTerminal/);
});
