...
import { activeTabStore, fromEditorTabId, isEditorTabId, toEditorTabId, useActiveTabId } from '../application/state/activeTabStore';
import { setPendingAiOpenTab } from '../application/state/pendingAiOpenTab';
...
interface TopTabsProps {
  ...
  onCreateLocalTerminal?: () => string | void;
}
...
const TopTabsInner: React.FC<TopTabsProps> = ({
  ...
  const handleToggleAiAssistant = useCallback(() => {
    const currentTabId = activeTabStore.getActiveTabId();
    const liveSessions = sessionsProp.filter((s) => !s.hiddenFromTabs);
    const liveWorkspaces = workspaces;
    const isTerminal = Boolean(
      currentTabId &&
      (liveSessions.some((s) => s.id === currentTabId) || liveWorkspaces.some((w) => w.id === currentTabId))
    );

    if (isTerminal) {
      window.dispatchEvent(new CustomEvent('netcatty:toggle-ai-panel'));
    } else {
      const existingTabId = liveSessions[0]?.id ?? liveWorkspaces[0]?.id;
      if (existingTabId) {
        setPendingAiOpenTab(existingTabId);
        activeTabStore.setActiveTabId(existingTabId);
        window.dispatchEvent(new CustomEvent('netcatty:open-ai-panel', { detail: { tabId: existingTabId } }));
      } else {
        const newSessionId = onCreateLocalTerminal?.();
        if (newSessionId) {
          setPendingAiOpenTab(newSessionId);
          activeTabStore.setActiveTabId(newSessionId);
          setTimeout(() => {
            window.dispatchEvent(new CustomEvent('netcatty:open-ai-panel', { detail: { tabId: newSessionId } }));
          }, 60);
          setTimeout(() => {
            window.dispatchEvent(new CustomEvent('netcatty:open-ai-panel', { detail: { tabId: newSessionId } }));
          }, 200);
        } else {
          window.dispatchEvent(new CustomEvent('netcatty:open-ai-panel', { detail: { createLocalIfEmpty: true } }));
        }
      }
    }
  }, [sessionsProp, workspaces, onCreateLocalTerminal]);
