...
import { consumePendingAiOpenTab } from '../../application/state/pendingAiOpenTab';
...
export function useTerminalLayerEffects(ctx: TerminalLayerEffectsContext) {
  ...
  // Listen for global AI panel toggle (from TopTabs button) and open requests
    useEffect(() => {
      const toggleHandler = () => handleToggleAiFromTopBar();
      const openHandler = (e: Event) => {
        const detail = (e as CustomEvent<{ tabId?: string }>).detail;
        const targetTabId = detail?.tabId ?? activeTabIdRef.current;
        if (!targetTabId || targetTabId === 'vault') return;
        handleSwitchSidePanelTab?.('ai', targetTabId);
      };

      window.addEventListener('netcatty:toggle-ai-panel', toggleHandler);
      window.addEventListener('netcatty:open-ai-panel', openHandler);
      return () => {
        window.removeEventListener('netcatty:toggle-ai-panel', toggleHandler);
        window.removeEventListener('netcatty:open-ai-panel', openHandler);
      };
    }, [handleToggleAiFromTopBar, handleSwitchSidePanelTab, activeTabIdRef]);

    // Open AI panel if requested while tab was pending or initializing
    useEffect(() => {
      if (activeTabId && activeTabId !== 'vault' && consumePendingAiOpenTab(activeTabId)) {
        handleSwitchSidePanelTab?.('ai', activeTabId);
      }
    }, [activeTabId, handleSwitchSidePanelTab]);
