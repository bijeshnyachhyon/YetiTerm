...
const TerminalLayerInner: React.FC<TerminalLayerProps> = ({
  ...
  const handleSwitchSidePanelTab = useCallback((tab: SidePanelTab, specificTabId?: string) => {
    const tabId = specificTabId ?? activeTabIdRef.current;
    if (!tabId || tabId === 'vault') return;
    const currentPanel = sidePanelOpenTabsRef.current.get(tabId);
