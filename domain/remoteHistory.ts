...
/** Marker prefix YetiTerm AI uses when executing commands via the PTY bridge. */
export const YETITERM_AI_HISTORY_MARKER = '__YTMCP_';
export const NETCATTY_AI_HISTORY_MARKER = '__NCMCP_';

/** True when a shell history line came from YetiTerm AI PTY exec, not the user. */
export function isYetiTermAiHistoryCommand(command: string): boolean {
  return command.includes(YETITERM_AI_HISTORY_MARKER) || command.includes(NETCATTY_AI_HISTORY_MARKER);
}
export const isNetcattyAiHistoryCommand = isYetiTermAiHistoryCommand;

const YETITERM_MANAGED_STARTUP_COMMAND =
  /^(?:sh\s+-c\s+.*printf .*\\033\[H\\033\[2J\\033\[3J.*(?:_nc|_yt)_docker_err=.*\bdocker\s+inspect\b|printf '\\033\[H\\033\[2J\\033\[3J';\s*(?:(?:_nc|_yt)_docker_err=.*\bdocker\s+inspect\b|exec\s+(?:docker\s+(?:exec|logs)\b|tmux\s+attach\b)))/;

/** True when a shell history line came from a YetiTerm-managed terminal launch. */
export function isYetiTermManagedStartupHistoryCommand(command: string): boolean {
  return YETITERM_MANAGED_STARTUP_COMMAND.test(command.trim());
}
export const isNetcattyManagedStartupHistoryCommand = isYetiTermManagedStartupHistoryCommand;
