...
export function resolveEffectiveAppLockRuntimeState(
  runtimeState: RuntimeAppLockState,
  settings: AppLockSettings,
): RuntimeAppLockState {
  const normalizedSettings = normalizeAppLockSettings(settings);
  const normalizedRuntime = normalizeRuntimeAppLockState(runtimeState);

  // If App Lock is not enabled or no password verifier is configured,
  // the app must NEVER be locked.
  ...
  if (normalizedRuntime.initialized) {
...
export function useAppLockState(settings: AppLockSettings) {
  ...
  const effectiveRuntimeState = useMemo(
    () => resolveEffectiveAppLockRuntimeState(runtimeState, normalizedSettings),
    [runtimeState, normalizedSettings],
  );
