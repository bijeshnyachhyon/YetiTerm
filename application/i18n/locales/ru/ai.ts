...
export const ruAiMessages: Messages = {
  ...
  'ai.providers.contextWindow.help': 'Оставьте пустым, чтобы использовать значение из списка моделей, если оно доступно; иначе YetiTerm применит безопасное значение по умолчанию.',
  ...
  'ai.codex.customConfigMissingEnvKey': 'Предупреждение: {envKey} не задана в переменных окружения вашей оболочки. Экспортируйте её (или запустите yetiterm из оболочки, где она задана), чтобы Codex мог пройти аутентификацию.',
  ...
  'ai.cursor.apiKeyEnvHint': 'Cursor может использовать CURSOR_API_KEY из shell. Сохраняйте ключ здесь только если хотите переопределить его в YetiTerm.',
  'ai.cursor.apiKeyOverrideHint': 'YetiTerm сначала использует сохранённый здесь ключ, затем CURSOR_API_KEY.',
  ...
  'ai.codebuddy.persistSession.hint': 'Сохраняет на диск собственную историю сессии CodeBuddy, чтобы CodeBuddy мог возобновить её позже. Отключите, чтобы запретить хранение сессии CodeBuddy; YetiTerm всё ещё может сохранять историю чата и метаданные внешней сессии. Контрольные точки файлов пропускаются. Требуется CodeBuddy CLI 2.125.1 или новее.',
  ...
  'ai.grok.runtime.acp.description':
    'Подключение к Grok через Agent Client Protocol (grok agent stdio). YetiTerm MCP внедряется в session/new. Выключите, чтобы использовать исходный headless streaming-json CLI.',
  ...
  'ai.toolAccess.mode': 'Режим доступа YetiTerm',
  'ai.toolAccess.description': 'Выберите, как внешние агенты получают доступ к сессиям YetiTerm. MCP предоставляет встроенный сервер, а Skills + CLI указывает агентам на локальный skill YetiTerm и команды CLI.',
  'ai.toolAccess.mode.mcp': 'MCP',
  'ai.toolAccess.mode.skills': 'Skills + CLI',
  'ai.toolAccess.mcpPrompt.title': 'Промпт для вашего ИИ-клиента',
  'ai.toolAccess.mcpPrompt.description': 'Вставьте этот промпт в ИИ-клиент (Codex, Claude Code и т. д.), и он сам зарегистрирует MCP YetiTerm.',
  ...
  'ai.toolAccess.skills.description': 'В режиме Skills + CLI агенты автоматически получают путь к этому локальному skill-файлу. Путь к лаунчеру YetiTerm CLI передаётся агенту в каждой сессии.',
  'ai.toolAccess.skills.unavailable': 'Путь к skill-файлу недоступен',

  'ai.externalMcp.title': 'Внешний MCP',
  'ai.externalMcp.description': 'Откройте YetiTerm как MCP-сервер для Codex, Claude Code, Cursor и Grok. Набор инструментов тот же, что у встроенных агентов. Держите YetiTerm запущенным, пока клиенты подключены.',
  ...
  'ai.externalMcp.mode.description': 'Временный режим отключается после простоя. Постоянный восстанавливает External MCP при запуске YetiTerm.',
  ...
  'ai.externalMcp.usage.keepRunning': '1. Включите External MCP и держите YetiTerm запущенным.',
  ...
  'ai.externalMcp.permissionMode.hint': 'Тот же параметр, что Settings → AI → Safety. Auto выполняет write-инструменты YetiTerm без запросов; Confirm спрашивает каждый раз. Внешние клиенты (Codex / Claude / Grok) могут показывать своё подтверждение инструментов.',
  ...
  'ai.externalMcp.codexAdded': 'Запись MCP для Codex и навык YetiTerm добавлены. Перезапустите Codex или откройте новую сессию.',
  'ai.externalMcp.claudeAdded': 'Запись MCP для Claude Code и навык YetiTerm добавлены. Перезапустите Claude Code или откройте новую сессию.',
  'ai.externalMcp.grokAdded': 'Запись MCP для Grok и навык YetiTerm добавлены. Перезапустите Grok или откройте новую сессию.',
  ...
  'ai.externalMcp.conflict.description': 'Запись yetiterm-external уже существует и указывает в другое место. Удалите или измените её вручную.',
  ...
  'ai.userSkills.description': 'Откройте папку skills YetiTerm, чтобы добавить свои каталоги skills. YetiTerm автоматически сканирует их и добавляет только лёгкие индексы, если skill явно не соответствует текущему запросу.',
  ...
  // AI Error
  'ai.codex.bridgeError': 'Обработчики главного процесса Codex ещё не загружены. Полностью перезапустите YetiTerm или dev-процесс Electron и попробуйте снова.',
  ...
  'ai.safety.permissionMode.description': 'Управляет тем, как AI взаимодействует с вашими терминалами. Режим наблюдателя блокирует все операции записи через YetiTerm и применяется как к встроенным, так и к внешним агентам. Режим подтверждения носит рекомендательный характер для внешних агентов (они управляют собственным потоком одобрения инструментов).',
  ...
  'ai.safety.blocklist.description': 'Regex-шаблоны для блокировки опасных команд. Применяется как к встроенным, так и к внешним агентам через механизм выполнения YetiTerm.',
  ...
  'ai.safety.note': 'Эти настройки безопасности применяются к действиям, выполняемым через YetiTerm. Внешние CLI-агенты могут иметь собственные локальные инструменты и собственные правила управления ими.',
