...
export const ruVaultMessages: Messages = {
  ...
  'vault.import.progress.keepOpen': 'Не закрывайте YetiTerm, пока импорт выполняется в фоне.',
  ...
  'pf.type.dynamic.desc': 'Динамический проброс портов превращает YetiTerm в SOCKS-прокси-сервер.',
  ...
  'settings.sshDeepLink.enable': 'Открывать ssh:// и telnet:// ссылки в YetiTerm',
  'settings.sshDeepLink.enableDesc': 'Разрешить YetiTerm обрабатывать ssh:// и telnet:// ссылки из браузеров и других приложений, а также запуски в стиле PuTTY, например -ssh user@host -P 22 -pw password.',
  'settings.explorerContextMenu.title': 'Проводник Windows',
  'settings.explorerContextMenu.enable': 'Показывать «Open in YetiTerm» в контекстном меню папок',
  ...
  'settings.jmsDeepLink.enableDesc': 'Зарегистрировать YetiTerm как обработчик клиентских ссылок JumpServer jms://. Может конфликтовать с официальным клиентом JumpServer.',
  ...
  'hostDetails.algorithms.inheritedNotice': 'В текущей группе заданы переопределения алгоритмов для: {categories}. Кнопка «Сбросить» здесь возвращает к спискам группы, а не к значениям YetiTerm по умолчанию. Чтобы игнорировать ограничение группы, очистите переопределение в настройках алгоритмов группы.',
