...
export const zhCNTerminalMessages: Messages = {
  ...
  'terminal.osc7Setup.desc': 'YetiTerm 会为当前远端用户添加 OSC 7 提示符配置。这样在 sudo 或 su 之后，SFTP 也能继续跟随终端目录。',
  ...
  'terminal.et.proxyUnsupported': 'EternalTerminal 目前不支持 YetiTerm 的代理设置。请改用 SSH，或移除该主机的代理。',
  'terminal.et.multiJumpUnsupported': 'EternalTerminal 目前在 YetiTerm 中最多支持一个跳板机。',
  ...
  'settings.sshDeepLink.enable': '用 YetiTerm 打开 ssh:// 和 telnet:// 链接',
  'settings.sshDeepLink.enableDesc': '允许 YetiTerm 接管来自浏览器和其他应用的 ssh://、telnet:// 链接，以及堡垒机常用的 PuTTY 启动参数（例如 -ssh user@host -P 22 -pw password）。',
  'settings.explorerContextMenu.title': 'Windows 资源管理器',
  'settings.explorerContextMenu.enable': '在文件夹右键菜单中显示 “Open in YetiTerm”',
  ...
  'settings.jmsDeepLink.enableDesc': '将 YetiTerm 注册为 JumpServer jms:// 客户端链接的处理程序，可能与官方 JumpServer 客户端冲突。',
  ...
  'settings.terminal.font.cjk.unavailableWarning': '本机没有安装这个字体。YetiTerm 会保留设置，并使用安全字体继续显示。',
  ...
  'snippets.import.toast.invalidDesc': '这不是有效的 YetiTerm 代码片段文件。',
  ...
  'snippets.import.modal.desc': '选择一个或多个 YetiTerm 代码片段 JSON 文件。导入前会先解析并展示结果，不会直接写入。',
