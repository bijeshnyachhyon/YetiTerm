...
import { setPendingAiOpenTab } from '../state/pendingAiOpenTab';
...
export function AppSideEffects() {
  ...
  // Handle request to open AI Assistant when no terminal session exists
  useEffect(() => {
    const handleOpenAiPanelEvent = (e: Event) => {
      const detail = (e as CustomEvent<{ tabId?: string; createLocalIfEmpty?: boolean }>).detail;
      if (detail?.createLocalIfEmpty) {
        const liveSessions = sessionsRef.current.filter((s) => !s.hiddenFromTabs);
        const liveWorkspaces = workspacesRefForHotkeys.current;
        if (liveSessions.length === 0 && liveWorkspaces.length === 0) {
          const newSessionId = handleCreateLocalTerminal();
          if (newSessionId) {
    ...
    window.addEventListener('netcatty:open-ai-panel', handleOpenAiPanelEvent);
    return () => window.removeEventListener('netcatty:open-ai-panel', handleOpenAiPanelEvent);
  }, [handleCreateLocalTerminal]);
