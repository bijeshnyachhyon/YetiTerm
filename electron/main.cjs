...
function showStartupError(err) {
  const title = "YetiTerm";
  const code = err && typeof err === "object" ? err.code : null;
  const message =
    code === "ENOENT"
      ? "Renderer files are missing. Please reinstall or rebuild YetiTerm."
      : "Failed to load the UI. Please relaunch YetiTerm.";
...
if (!gotLock) {
  ...
  app.on("before-quit", (event) => {
    const { ipcMain: _ipcMain, dialog: _dialog } = electronModule;
