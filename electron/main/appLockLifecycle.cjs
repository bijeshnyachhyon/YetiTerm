...
const { canLockFromSettings } = require("../bridges/appLockSettingsStore.cjs");
...
function ensureAppLockForFreshSession(appLockController, reason = "startup") {
  if (!shouldBackgroundLockOnHide(appLockController)) return false;
  if (typeof appLockController.getSettings === "function") {
    try {
      const settings = appLockController.getSettings();
      if (settings && !canLockFromSettings(settings)) {
        return false;
      }
    } catch {
      // ignore
...
async function handleBeforeQuit({
  ...
  try {
    ...
    if (dirtyWindows.length === 0) {
      const isQuittingForUpdate = Boolean(windowManager?.isQuittingForUpdate?.());
      let confirmQuitPrompt = true;
      if (!isQuittingForUpdate) {
        try {
          const electronDialog = injectedDialog || require("electron")?.dialog;
          if (electronDialog && typeof electronDialog.showMessageBox === "function") {
            const focusedWin = queryableWindows.find((w) => !w.isDestroyed?.() && w.isFocused?.()) || queryableWindows[0];
            const result = await electronDialog.showMessageBox(focusedWin || undefined, {
              type: "question",
              buttons: ["Yes", "No"],
              defaultId: 0,
              cancelId: 1,
              title: "Quit YetiTerm",
              message: "Are you sure you want to quit YetiTerm?",
              detail: "Any active terminal sessions will be terminated.",
            });
            confirmQuitPrompt = result?.response === 0;
          }
        } catch {
          // If dialog throws or missing in test environments, allow quit to proceed
        }
      }

      if (!confirmQuitPrompt) {
        setQuitGuardChannelBusy?.(false);
        return { committed: false, skipped: "cancelled" };
      }
