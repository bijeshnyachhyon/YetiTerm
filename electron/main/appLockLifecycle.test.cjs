...
test("handleBeforeQuit prompts user to confirm quit and proceeds when Yes (0) is chosen", async () => {
  const calls = [];
  const mainWindow = {
    isDestroyed: () => false,
    isVisible: () => true,
  ...
  const dialog = {
    showMessageBox: async (_win, options) => {
      calls.push(`dialog:${options.title}:${options.buttons.join(",")}`);
...
test("handleBeforeQuit prompts user to confirm quit and cancels when No (1) is chosen", async () => {
  const calls = [];
  const mainWindow = {
    isDestroyed: () => false,
    isVisible: () => true,
