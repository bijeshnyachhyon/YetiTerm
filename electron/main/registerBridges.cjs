...
function createBridgeRegistrar(context) {
  ...
  const registerBridges = (win) => {
    ...
    credentialBridge.registerHandlers(ipcMain, electronModule, { appLockController, getAppLockController });
    ...
    ipcMain.handle("netcatty:openPath", async (_event, targetPath) => {
      ...
      try {
        const { shell } = electronModule;
        const error = await shell.openPath(path.resolve(targetPath.trim()));
