...
if (isTrustedRendererLocation(allowedOrigins)) {
  const bridgePayload = { ...existing, ...api, ...figSpecApi };
  contextBridge.exposeInMainWorld("yetiterm", bridgePayload);
  contextBridge.exposeInMainWorld("netcatty", bridgePayload);
} else {
  // If a window navigates to an untrusted origin, do NOT expose the bridge.
  try {
    console.warn("[Preload] Refusing to expose yetiterm bridge to untrusted origin:", window?.location?.origin);
  } catch {
    // ignore
  }
}
