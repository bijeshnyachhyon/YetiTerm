...
async function readLocalFile(event, payload) {
  const filePath = validateLocalPath(payload?.path);
  const maxBytes =
...
async function writeLocalFile(event, payload) {
  ...
  if (!payload?.content && payload?.content !== "") {
    throw new Error("Invalid content for writeLocalFile");
  }
  await fs.promises.writeFile(filePath, Buffer.from(payload.content));
  return true;
}
...
async function renameLocalFile(event, payload) {
  const oldPath = validateLocalPath(payload?.oldPath);
  const newPath = validateLocalPath(payload?.newPath);
...
async function mkdirLocal(event, payload) {
  const dirPath = validateLocalPath(payload?.path);
  try {
