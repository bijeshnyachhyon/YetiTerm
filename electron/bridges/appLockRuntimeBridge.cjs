...
function createAppLockController({
  ...
  function enforceWindowProtection(win) {
    ...
    try {
      if (typeof win.getTitle === "function") {
        ...
        if (currentTitle && currentTitle !== "YetiTerm") {
          lockedWindowTitles.set(win, currentTitle);
        }
      }
      win.setTitle?.("YetiTerm");
  ...
  function setWindowTitle(win, title) {
    ...
    const nextTitle = typeof title === "string" && title.trim() ? title.trim() : "YetiTerm";
    try {
  ...
  function protectWindowsForRuntimeState(nextState) {
    ...
    for (const [win, title] of lockedWindowTitles) {
      try {
        if (!win.isDestroyed?.() && win.getTitle?.() === "YetiTerm") {
          win.setTitle?.(title || "YetiTerm");
        }
      } catch {
  ...
  return {
    getSettings,
    getRuntimeState,
    isLocked: () => getRuntimeState()?.locked === true,
