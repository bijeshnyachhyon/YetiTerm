...
test("decrypt IPC handler withholds plaintext when app lock is active", async () => {
  ...
  const storage = fakeSafeStorage();
  let locked = true;
  const appLockController = { isLocked: () => locked };
  registerHandlers(ipcMain, { safeStorage: storage }, { appLockController });

  const enc = encryptCredentialValue("my-secret", storage);
  const decryptHandler = handlers.get("netcatty:credentials:decrypt");
  const result = await decryptHandler({}, enc);
  assert.equal(result, enc); // withholds plaintext while locked!

  locked = false;
  const unlockedResult = await decryptHandler({}, enc);
  assert.equal(unlockedResult, "my-secret");
});
