...
// Stable directory under user temp or home for YetiTerm files
const YETITERM_TEMP_DIR_NAME = "YetiTerm";
const NETCATTY_TEMP_DIR_NAME = YETITERM_TEMP_DIR_NAME;
...
function configureToolOutputSigningKey(electronModule) {
  ...
  toolOutputSigningKeyPromise = null;
}

async function getToolOutputSigningKey({ retry = true } = {}) {
  const generation = tempDirRebindGeneration;
  if (!toolOutputSigningKeyPromise && toolOutputSafeStorage) {
    toolOutputSigningKeyPromise = loadOrCreateToolOutputSigningKey(toolOutputSafeStorage);
  }
  const key = toolOutputSigningKeyPromise ? await toolOutputSigningKeyPromise.catch(() => null) : null;
