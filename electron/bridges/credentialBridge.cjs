 ...
 * @param {{ appLockController?: { isLocked?: () => boolean } | null, getAppLockController?: () => ({ isLocked?: () => boolean } | null) }} [options]
 */
function registerHandlers(ipcMain, electronModule, options = {}) {
  ...
  ipcMain.handle("netcatty:credentials:decrypt", (_event, value) => {
    const isLocked = Boolean(
      options?.appLockController?.isLocked?.()
      || options?.getAppLockController?.()?.isLocked?.()
    );
    if (isLocked) {
      return value;
    }
